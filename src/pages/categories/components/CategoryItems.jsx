...
function CategoryItems({ active }) {
    ...
    const handleAddOrder = () => {
        dispatch(addFavorite(active))
    }

    const [activeButton, setActiveButton] = useState(null);

    // Elementni `active` qilish funksiyasi
    const handleClick = (buttonIndex) => {
        // activeButton holatini yangilash
                    ...
                    <div className={active ? 'category-item active' : 'category-item'} key={item.id}>
                        ...
                        <div className={active ? "category-item__content active" : "category-item__content"}>
                            ...
                            <div className={active ? "category-item__prices active" : "category-item__prices"}>
                                <p className={active ? "category-item__price active" : "category-item__price"}>{item.price.toLocaleString()} so'm</p>
                                <p className={active ? "category-item__oldprice active" : "category-item__oldprice"}>{item.oldPrice ? item.oldPrice.toLocaleString() : null}</p>
                            </div>
                            ...
                            <div className={active ? "category-item__row active" : "category-item__row"}>
                                <div className={active ? "category-item__row__col active" : "category-item__row__col"}>
                                    ...
                                    <div className={active ? "category-item__order active" : "category-item__order"}>
                                        <p className={active ? "category-item__orders active" : "category-item__orders"}>{item.sold} Buyurtmalar </p>
                                    </div>
                                    <p className={active ? "category-item__text active" : "category-item__text"}> Yetkazib berish bepul</p>
                                </div>
                            </div>
                            <p className={active ? "category-item__subtitle active" : "category-item__subtitle"}>{item.desc?.substring(0, 120)}...</p>
                            <Link to={`/product/${item.slug}`}>
                                <button className={active ? 'category-item__btn active' : 'category-item__btn'}>Mahsulotni ko'rish</button>
                            </Link>
                        </div>
                        </Link>
                        
                        <button 
                            className={activeButton === 1 ? 'favourite-items__btn active' : 'favourite-items__btn'}
